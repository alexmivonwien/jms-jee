...
/**
 * 
 * Implicitly implements the remote interface for the MessageProducerJob session bean. Used by remote stand-alone clients to schedule the message production All settings related to the schedule are
 * stored within the current application
 * 
 * @author Alexander
 *
 *
 *         Session bean used to produce messages (events) on a particular time basis. It uses the JEE TimerService, so that messages are produced in repeating series Producing of messages is done via
 *         JMS, and the types of events produced is determined by an AbstarctEventFactory
 * 
 * @author Alexander
 *
 */
...
@Stateless
public class MessageProducerJob {// implements MessageProducerJobRemoteXYZ
