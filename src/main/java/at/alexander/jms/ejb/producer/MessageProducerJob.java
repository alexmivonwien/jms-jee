...
/**
 * Session bean used to produce messages (events) on a particular time basis.
 * It uses the JEE TimerService, so that messages are produced in repeating series
 * Producing of messages is done via JMS, and the types of events produced is determined
 * by an AbstarctEventFactory
 * 
 * @author Alexander
 *
 */
