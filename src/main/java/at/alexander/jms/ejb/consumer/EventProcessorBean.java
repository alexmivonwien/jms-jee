...
@EJBs({
		...
		@EJB(name = "ejb/LogDB", beanInterface = Command.class, beanName = "LogDB"),

		@EJB(name = "ejb/LogFile", beanInterface = Command.class, beanName = "LogFile"),

		@EJB(name = "ejb/SendMail", beanInterface = Command.class, beanName = "SendMail")

})

/**
 * The EventProcessorBean is used to process the events that have been consumed by the MessageConsumerMDB. It does so by querying the DB about what actions correspond to the particular event recieved,
 * looking up the corresponding action implementations from JNDI and calling the actions' execute method, which actually does the work.
 * 
 */
